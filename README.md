...
- **운영 대시보드**: OHLC 캔들·거래량 차트, 최근 봉, 데이터 완전성, 연결 상태와 장기 백필 진행률·보유 범위를 실시간으로 표시한다. 6시간봉은 UTC 날짜 경계와 `06`·`12`·`18`시를 구분하며, 최근 봉은 `더보기`로 이전 8개씩 누적하고 로컬 데이터를 소진한 시점에만 커서로 추가 조회한다. 장기 백필이 완료되면 현재 차트 snapshot을 자동 갱신하며, SSE 단절 시에는 지수 백오프로 복구한 뒤 최신 화면을 한 번 재동기화한다.
...
- 6시간봉의 UTC 날짜·6시간 눈금과 봉 간격을 구분할 수 있는 초기 표시 범위
- 차트 왼쪽 경계와 최근 봉 `더보기`를 통한 cursor 기반 과거 데이터 추가 로딩
- 장기 백필 완료 이벤트에 맞춘 현재 차트·최근 봉 snapshot 자동 갱신
...
- 6시간봉 UTC 날짜·시간 눈금과 초기 표시 범위, 최근 봉의 8개 단위 누적 `더보기`와 로컬 우선 cursor 조회 (`apps/web/src/**/*.test.ts(x)`)
- 백필 완료 전이의 최신 candle snapshot 교체, 반복 이벤트 중복 요청과 이전 진행 중 요청 공유 방지 (`apps/web/src/**/*.test.ts(x)`)
- 백필 진행률·coverage·재시도·영구 실패 상태, 금융 차트와 최근 봉 테이블의 접근 가능한 상태 표시 (`apps/web/src/**/*.test.ts(x)`)
