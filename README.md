...
- **운영 대시보드**: OHLC 캔들·거래량 차트, 최근 봉, 데이터 완전성, 연결 상태와 장기 백필 진행률·보유 범위를 실시간으로 표시한다. 6시간봉은 UTC 날짜 경계와 `06`·`12`·`18`시를 구분하며, 최근 봉은 8개씩 로컬 데이터를 탐색한 뒤 필요한 시점에만 커서로 과거 데이터를 추가 조회한다. SSE 단절 시에는 지수 백오프로 자동 복구하고 최신 화면을 한 번 재동기화한다.
...
- 차트 왼쪽 경계와 테이블의 8개 단위 이전·최신 방향 탐색을 통한 cursor 기반 과거 데이터 추가 로딩
...
- 6시간봉 UTC 날짜·시간 눈금과 초기 표시 범위, 최근 봉의 8개 단위 로컬 우선·cursor 페이지 탐색 (`apps/web/src/**/*.test.ts(x)`)
