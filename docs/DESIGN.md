...
이 문서는 [`REQUIREMENTS.md`](REQUIREMENTS.md)의 요구사항을 구현하기 위한 기술 선택과 현재 동작을 정의한다.
...
서버 프로세스 하나가 수집, 최근 구간 복구, 장기 백필, 집계와 API 제공을 담당한다. 수집된 1분봉은 SQLite에 먼저 반영한 뒤 클라이언트로 전송한다. 6시간봉과 일봉은 저장된 1분봉을 UTC 시간 버킷으로 조회 시 집계한다. 장기 백필은 실시간 전환 후 별도 worker에서 실행하며 진행 위치를 DB에 저장한다.
...
페이지를 반영할 때마다 `cursor` 필드에 다음 백필 처리 위치와 진행 상태를 같은 SQLite에 저장한다. 서버가 중단되면 `pending`, `running`, `retrying` 작업을 마지막으로 저장된 위치부터 재개한다.
...
2. DB의 마지막 확정 봉을 조회해 최근 누락 구간과 `policy.backfill.warmupHours`를 우선 REST 백필한다.
3. 버퍼 이벤트를 시간순으로 upsert하고 실시간 모드로 전환한다. HTTP API와 ready 상태는 이 시점부터 장기 백필 완료를 기다리지 않는다.
4. 남은 `policy.backfill.days` 구간은 최신 시각에서 과거 방향으로 백그라운드 worker가 채운다.

WebSocket을 먼저 연결하므로 백필 도중 발생하는 최신 이벤트를 놓치지 않는다. 기본 백필·보존 기간은 365일, warmup은 24시간이며 `apps/server/src/config/policy.ts`에서 관리한다. 보존 기간이 목표 백필 기간보다 짧으면 서버 시작 전에 정책 오류로 거부한다.
...
- 종목별 job을 하나씩 처리하고 Binance 페이지 단위로 candle과 다음 백필 처리 위치를 저장한다.
...
- 연속 재시도가 기본 12회를 넘거나 잘못된 요청처럼 재시도로 해결되지 않는 오류면 `failed`로 확정하고 백필 진행 위치와 오류를 보존한다.
- 실패는 구조화 로그와 알림 포트로 전달한다. 외부 알림 연동은 어댑터로 확장할 수 있으며, 운영자는 원인을 해결한 뒤 종목별 CLI로 job을 수동 재개한다.
- 종료 신호는 진행 중 페이지 또는 재시도 대기를 중단하고 DB에 저장된 백필 진행 위치를 보존한다.
...
5. **가격·거래량 차트**: OHLC 캔들과 거래량, 서버 설정 종목 및 1분봉/6시간봉/일봉 전환, 왼쪽 경계의 과거 데이터 지연 로딩. 6시간봉은 UTC 날짜 경계와 `06`·`12`·`18`시를 구분할 수 있는 초기 범위를 사용한다.
6. **최근 봉 테이블**: 선택한 봉 주기의 시각, OHLC, 거래량, 체결 수와 확정 여부를 최신 8개부터 표시하고 `더보기`로 이전 8개씩 누적한다.
...
- 최근 봉 테이블은 이미 받은 데이터를 최신 8개부터 표시하고 `더보기`마다 이전 8개를 아래에 누적한다. 로컬 데이터를 모두 표시한 뒤에만 `page.nextBefore`로 다음 페이지를 요청하며, 종목이나 봉 주기를 바꾸면 최신 8개로 초기화한다.
- 선택 종목의 장기 백필 상태가 `completed`로 전환되면 현재 봉 주기의 최신 candle page를 다시 조회해 차트와 최근 봉을 DB snapshot으로 교체한다. 반복된 완료 상태 이벤트는 추가 조회를 만들지 않는다.
- 더 과거 데이터가 아직 DB에 없고 장기 백필이 진행 중이면 빈 데이터 대신 `과거 데이터 백필 중`으로 안내한다. coverage가 확장되면 같은 `page.nextBefore` 값으로 다시 조회할 수 있다.
...
apps/
  server/                 # 수집기, 복구 로직, REST/SSE API
    ...
    src/config/           # runtime 환경 설정, 제품 정책과 공용 시간 상수
  web/                    # React 대시보드
packages/
...
실제 값은 각 앱의 `.env`에 두고, 저장소에는 같은 키와 설명을 가진 `apps/server/.env.example`, `apps/web/.env.example`만 커밋한다. server, migration, 운영 CLI는 실행 위치와 관계없이 서버 설정 파일을 읽고 Vite는 웹 설정 파일을 읽는다. 배포마다 달라지는 서버 인프라 값만 `config/runtime.ts`가 환경변수로 읽는다. 수집 종목, 백필·보존 기간, page size, 재시도·재연결, API limit과 SSE heartbeat는 `config/policy.ts`의 고정 제품 정책이며 변경 시 재빌드·재배포한다. 여러 모듈이 공유하는 시간 단위는 `config/time.ts`에서 관리하고 외부 모듈은 `config/index.ts`를 통해 가져온다. Binance 공개 시세 API만 사용하므로 API key는 필요하지 않다.
...
8. **비차단 장기 백필과 과거 탐색**: DB에 진행 위치를 저장하는 백필 job, 진행·재시도 상태, candle 페이지네이션과 이벤트 중심 UI 동기화를 추가한다.
9. **차트 탐색과 실패 정책**: 6시간봉 UTC 눈금, 최근 봉 8개 단위 탐색, 백필 연속 재시도 한도·실패 알림 포트·수동 재개와 config 책임 분리를 추가한다.
10. **최근 봉과 백필 완료 동기화**: 최근 봉 탐색을 누적 `더보기`로 단순화하고 백필 완료 시 최신 candle snapshot을 자동 반영한다.
11. **실행 환경 정합성**: Node.js 설치부터 시작하는 Quick Start와 서버·웹 진입점의 앱별 `.env` 로딩을 통일한다.

1~4단계의 백엔드 구현은 Claude가, 5단계는 Codex가 담당했다. 6~11단계는 각자 소유 영역을 나누어 완료했다.

## 12. 완료 기준

- 새 DB에서 실행하면 최근 24시간을 우선 사용할 수 있고 두 종목의 365일 1분봉이 백그라운드에서 최신→과거 방향으로 채워진다.
- 장기 백필은 HTTP ready와 실시간 수집을 막지 않으며 서버 재시작 후 DB에 저장된 진행 위치부터 이어진다.
...
- 차트와 테이블은 `page.nextBefore`로 과거 데이터를 추가하며 중복이나 보던 시간 위치의 점프가 없다.
- 6시간봉은 UTC 날짜 경계와 6시간 간격을 구분하고, 최근 봉은 `더보기`로 로컬 데이터를 8개씩 누적한 뒤 필요한 시점에만 다음 페이지를 요청한다.
- 장기 백필 완료 전이에 현재 candle page를 다시 조회하며 같은 완료 이벤트를 반복 처리하지 않는다.
- 장기 백필 진행률, 실제 보유 범위, 재시도·실패 상태를 실시간 수집 상태와 분리해 표시한다.
- 장기 백필은 연속 재시도 한도를 넘으면 원인과 DB에 저장된 진행 위치를 보존한 채 실패하고 운영자가 수동 재개할 수 있다.
- 운영 완전성은 선택한 차트 봉 주기와 무관하게 최근 24시간 원본 1분봉을 기준으로 표시된다.
- 보존 기간이 지난 데이터가 실시간 수집을 방해하지 않고 정리된다.
- 잘못된 런타임 환경변수와 제품 정책 불변조건은 서버 시작 전에 명확한 오류로 거부된다.
