...
- 연속 재시도가 기본 12회를 넘거나 잘못된 요청처럼 재시도로 해결되지 않는 오류면 `failed`로 확정하고 cursor와 오류를 보존한다.
...
- 최근 봉 테이블은 이미 받은 데이터를 최신 8개부터 표시하고 `더보기`마다 이전 8개를 아래에 누적한다. 로컬 데이터를 모두 표시한 뒤에만 cursor API를 요청하며, 종목이나 봉 주기를 바꾸면 최신 8개로 초기화한다.
...
실제 값은 `.env`에 두고, 저장소에는 같은 키와 설명을 가진 `.env.example`만 커밋한다. 배포마다 달라지는 인프라 값만 `config/runtime.ts`가 환경변수로 읽는다. 수집 종목, 백필·보존 기간, page size, 재시도·재연결, API limit과 SSE heartbeat는 `config/policy.ts`의 고정 제품 정책이며 변경 시 재빌드·재배포한다. 여러 모듈이 공유하는 시간 단위는 `config/time.ts`에서 관리하고 외부 모듈은 `config/index.ts`를 통해 가져온다. Binance 공개 시세 API만 사용하므로 API key는 필요하지 않다.
...
1~4단계의 백엔드 구현은 Claude가, 5단계는 Codex가 담당했다. 6~9단계는 각자 소유 영역을 나누어 완료했다.
...
- 6시간봉은 UTC 날짜 경계와 6시간 간격을 구분하고, 최근 봉은 `더보기`로 로컬 데이터를 8개씩 누적한 뒤 필요한 시점에만 cursor 요청을 보낸다.
- 장기 백필 완료 전이에 현재 candle page를 다시 조회하며 같은 완료 이벤트를 반복 처리하지 않는다.
- 장기 백필 진행률, 실제 보유 범위, 재시도·실패 상태를 실시간 수집 상태와 분리해 표시한다.
- 장기 백필은 연속 재시도 한도를 넘으면 원인과 cursor를 보존한 채 실패하고 운영자가 수동 재개할 수 있다.
