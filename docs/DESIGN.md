...
서버 프로세스 하나가 수집, 복구, 집계, API 제공을 담당한다. 수집된 1분봉은 SQLite에 먼저 반영한 뒤 클라이언트로 전송한다. 6시간봉과 일봉은 저장된 1분봉을 UTC 시간 버킷으로 조회 시 집계한다. 브라우저는 REST로 초기 화면을 채우고 SSE를 실시간 변경 신호로 사용한다.
...
3. 데이터가 없으면 `BACKFILL_DAYS` 전부터, 있으면 마지막 확정 봉의 다음 1분부터 REST 백필한다.
4. 마지막으로 완료된 1분봉까지 페이지 단위로 조회해 upsert한다.
5. 버퍼 이벤트를 시간순으로 upsert하고 실시간 모드로 전환한다.

WebSocket을 먼저 연결하므로 백필 도중 발생하는 최신 이벤트를 놓치지 않는다. 기본 `BACKFILL_DAYS`는 30일이며 환경변수로 변경할 수 있다.
...
- 원본 1분봉은 기본 30일간 보존한다. 만료 데이터는 실시간 수집과 분리된 배치로 삭제하며 실패해도 수집을 중단하지 않는다.
...
3. **시장 요약 카드**: 현재가, 최근 1시간 등락률, 최근 1시간 거래대금
4. **가격·거래량 차트**: OHLC 캔들과 거래량, 서버 설정 종목 및 1분봉/6시간봉/일봉 전환
5. **최근 봉 테이블**: 선택한 봉 주기의 시각, OHLC, 거래량, 체결 수, 확정 여부
...
| GET | `/api/status` | 종목별 수집 상태, 지연, 백필 결과와 24시간 완전성 |
...
`symbol`은 허용 목록으로 검증한다. `interval`은 `1m`, `6h`, `1d`만 허용하며 기본값은 `1m`이다. `limit` 기본값은 500, 최대값은 2,000이고 집계 후 반환할 봉 개수에 적용한다. 응답은 `{ "symbol", "interval", "candles" }`, 오류는 `{ "error": { "code", "message" } }` 형태로 통일한다.
...
7. **다중 봉과 운영 강화**: UTC 집계, 30일 보존, 백필·완전성 관측, health 분리와 OHLC 차트를 추가한다.

1~4단계의 백엔드 구현은 Claude가, 5단계는 Codex가 담당했다. 6단계와 7단계는 각자 소유 영역을 나누어 완료했다.

## 12. 완료 기준

- 새 DB에서 실행하면 두 종목의 최근 30일 1분봉이 채워지고 실시간 수집이 이어진다.
