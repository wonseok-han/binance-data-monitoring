...
6. **최근 봉 테이블**: 선택한 봉 주기의 시각, OHLC, 거래량, 체결 수와 확정 여부를 8개씩 표시하고 이전·최신 방향으로 탐색한다.
...
- 최근 봉 테이블은 이미 받은 데이터를 8개 단위로 먼저 탐색한다. 가장 오래된 로컬 페이지에서만 cursor API를 요청하며, 과거 데이터가 병합돼도 현재 페이지를 유지한다. 종목이나 봉 주기를 바꾸면 최신 8개로 초기화한다.
...
- 6시간봉은 UTC 날짜 경계와 6시간 간격을 구분하고, 최근 봉은 로컬 데이터를 8개씩 먼저 탐색한 뒤 필요한 시점에만 cursor 요청을 보낸다.
